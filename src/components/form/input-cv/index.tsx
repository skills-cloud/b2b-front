...
import { cv, IGetCvListFilters } from 'adapter/api/cv';
...
const methods = {
    [ERequestType.Cv]: {
        multi: cv.endpoints.getCvList,
        list : cv.endpoints.getCvList
    }
};
...
const InputCv = (props: IProps & typeof defaultProps) => {
            ...
            .then(({ data: loadData }) => {
                if(loadData?.results?.length) {
                    const res = loadData.results.map((item) => {
                        let name = item.last_name;

                        if(item.first_name) {
                            name = `${name} ${item.first_name.substring(0, 1).toUpperCase()}.`;
                        }
...
InputCv.requestType = ERequestType;

InputCv.defaultProps = defaultProps;
